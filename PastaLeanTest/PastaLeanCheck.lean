import Lean
import PastaLeanTest.MiniRegex

open Lean
open PastaLeanTest.MiniRegex

namespace PastaLeanTest
...
def parseSpecFromPython (pyPath : System.FilePath) (source : String) : Except String CheckSpec := do
  ...
  for rawLine in source.splitOn "\n" do
    ...
    if line == "PASTALEAN CHECK START" then
      if inBlock then
        throw s!"{pyPath}:{lineNo}: nested PASTALEAN CHECK START is not allowed."
      if sawStart then
        throw s!"{pyPath}:{lineNo}: multiple PASTALEAN CHECK blocks are not supported."
      inBlock := true
      sawStart := true
      continue
    if line == "PASTALEAN CHECK END" then
      if !inBlock then
        throw s!"{pyPath}:{lineNo}: PASTALEAN CHECK END without START."
      inBlock := false
      sawEnd := true
      continue
  ...
  if inBlock then
    throw s!"{pyPath}: missing PASTALEAN CHECK END."
  if !sawStart then
    throw s!"{pyPath}: missing PASTALEAN CHECK START block."
  if !sawEnd then
    throw s!"{pyPath}: missing PASTALEAN CHECK END."
  if spec.check.isEmpty && spec.checkErr.isEmpty && spec.checkExact.isNone && spec.checkErrExact.isNone then
    throw s!"{pyPath}: no CHECK directives found inside PASTALEAN CHECK block."
  return spec
...
`import PastaLean` / `import Libraries` / `open` preamble), so it is emitted verbatim — adding
...
def compileGeneratedLean (pyPath : System.FilePath) (target generated : String) :
  ...
  let tmpPath := System.FilePath.mk s!"/tmp/PastaLean_palc_{stamp}.lean"
...
def defaultCasesDir : System.FilePath :=
  System.FilePath.mk "PastaLeanTest/PASTALEAN CHECK/Cases"
...
def printUsage : IO Unit := do
  IO.println "PASTALEAN CHECK (PALC) - Testing framework for Python to Lean translation"
  IO.println "Usage:"
