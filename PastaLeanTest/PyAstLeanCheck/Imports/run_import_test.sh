...
#
# Run from the repository root:  bash PastaLeanTest/PastaLeanCheck/Imports/run_import_test.sh
set -uo pipefail
...
# `import Mathlib_module`. Place the converted defining module under the PastaLean lib so
...
DEF_LEAN="PastaLean/PyGens/Mathlib_module.lean"
USE_LEAN="/tmp/PastaLean_importer_check.lean"

cleanup() {
  rm -f "$DEF_LEAN" "$USE_LEAN" "$USE_LEAN".gen
  rm -f .lake/build/lib/lean/PastaLean/PyGens/Mathlib_module.olean \
        .lake/build/lib/lean/PastaLean/PyGens/Mathlib_module.ilean 2>/dev/null
}
# trap cleanup EXIT
...
if ! lake build PastaLean.PyGens.Mathlib_module >/dev/null 2>&1; then
...
cat > "$USE_LEAN" <<'LEAN'
import PastaLean
import PastaLean.PyGens.Mathlib_module
open PastaLean
